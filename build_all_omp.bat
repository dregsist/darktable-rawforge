...
echo Building test_menon.exe with /openmp ...
"%CL_PATH%" /nologo /O2 /openmp /TC /W3 /I"%MSVC_INC%" /I"%SDK_UCRT_INC%" /I"%SDK_UM_INC%" /I"%SDK_SH_INC%" /I"%WORK%" test_menon.c /Fe:test_menon.exe /link /LIBPATH:"%MSVC_LIB%" /LIBPATH:"%SDK_UCRT_LIB%" /LIBPATH:"%SDK_UM_LIB%"
if errorlevel 1 ( echo BUILD FAILED: test_menon.exe & exit /b 1 )
echo BUILD OK: test_menon.exe

echo Building test_rcd.exe with /openmp ...
"%CL_PATH%" /nologo /O2 /openmp /TC /W3 /I"%MSVC_INC%" /I"%SDK_UCRT_INC%" /I"%SDK_UM_INC%" /I"%SDK_SH_INC%" /I"%WORK%" test_rcd.c /Fe:test_rcd.exe /link /LIBPATH:"%MSVC_LIB%" /LIBPATH:"%SDK_UCRT_LIB%" /LIBPATH:"%SDK_UM_LIB%"
if errorlevel 1 ( echo BUILD FAILED: test_rcd.exe & exit /b 1 )
echo BUILD OK: test_rcd.exe

echo Building test_amaze.exe with /openmp ...
"%CL_PATH%" /nologo /O2 /openmp /TP /W3 /EHsc /I"%MSVC_INC%" /I"%SDK_UCRT_INC%" /I"%SDK_UM_INC%" /I"%SDK_SH_INC%" /I"%WORK%" /I"%WORK%\stub_headers" test_amaze.cc /Fe:test_amaze.exe /link /LIBPATH:"%MSVC_LIB%" /LIBPATH:"%SDK_UCRT_LIB%" /LIBPATH:"%SDK_UM_LIB%"
