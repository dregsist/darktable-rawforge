/*
 * Standalone RCD demosaic test harness.
 * Reads a binary float32 CFA file, runs rcd_demosaic(), writes binary float32 RGBA output.
 *
 * Usage:
 *   test_rcd <width> <height> <filters_hex> <input.bin> <output.bin>
 *
 * Input  (.bin): width*height float32 values, CFA mosaic in [0,1]
 * Output (.bin): width*height*4 float32 values, RGBA (R,G,B,0)
 *
 * Filters values (uint32 hex):
 *   RGGB = 0x94949494  GRBG = 0x61616161
 *   GBRG = 0x49494949  BGGR = 0x16161616
 */

#include "stubs.h"

/* basics.c provides pre_median (needed by ppg.c) */
#include "../darktable/src/iop/demosaicing/basics.c"
/* PPG needed for RCD border handling */
#include "../darktable/src/iop/demosaicing/ppg.c"
/* RCD implementation */
#include "../darktable/src/iop/demosaicing/rcd.c"
...
int main(int argc, char *argv[])
{
  if(argc != 6)
  {
    fprintf(stderr, "Usage: %s <width> <height> <filters_hex> <input.bin> <output.bin>\n", argv[0]);
  ...
  /* Run RCD demosaic; scaler=1.0 since CFA is in [0,1] */
  rcd_demosaic(out_buf, in_buf, width, height, filters, 1.0f);
  ...
  fprintf(stderr, "OK: %dx%d filters=0x%08X -> %s\n", width, height, filters, out_path);
  free(in_buf);
  free(out_buf);
  return 0;
}
